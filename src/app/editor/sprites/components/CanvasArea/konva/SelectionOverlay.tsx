...
export const SelectionOverlay = ({
  ...
  // Create rects for floating pixels
  const rects = []
  ...
  // Base coordinates based on selection.rect
  const baseX = startX + selection.rect.x * zoom
  const baseY = startY + selection.rect.y * zoom

  return (
    <Layer>
      {/* Floating Group: always present but only visible/active if floatingPixels exist */}
      <Group
        ref={groupRef}
        x={baseX}
        y={baseY}
      ...
      >
        {/* Transparent hit area — lets the user drag from anywhere inside the bounding box,
            not only from painted pixels */}
        <Rect
          x={0}
          y={0}
      ...
      {/* Transformer: only attached if floatingPixels exist */}
      {selection.floatingPixels && (
        <Transformer
          ref={trRef}
      ...
      {/* Marquee rect: only visible if NOT floating */}
      <Rect
        visible={!selection.floatingPixels}
        x={baseX}
        y={baseY}
